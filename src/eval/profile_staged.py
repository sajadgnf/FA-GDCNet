...
def _profile_mclip(samples: list[tuple[str, str]]) -> dict:
    import torch
    from PIL import Image

    from inference import models

    bundle = models.load_mclip_only()
    torch.cuda.reset_peak_memory_stats()
    timings: list[float] = []
    try:
        for caption, image_path in samples:
            ...
            torch.cuda.synchronize()
            t0 = time.perf_counter()
            emb_t = models.embed_text_mclip(bundle, caption)
        ...
        models.release(bundle)
    peak = int(torch.cuda.max_memory_allocated())
    return _stage_result("mclip", timings, peak)
...
def main(argv: list[str] | None = None) -> int:
    ...
    stages = [_profile_caption(samples), _profile_mclip(samples), _profile_polarity(samples)]
